...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 6) {
		...
		card->ext_csd.life_time_est_typ_a = ext_csd[EXT_CSD_DEVICE_LIFE_TIME_EST_TYP_A];
		card->ext_csd.life_time_est_typ_b = ext_csd[EXT_CSD_DEVICE_LIFE_TIME_EST_TYP_B];
	} else {
		card->ext_csd.data_sector_size = 512;
	}
